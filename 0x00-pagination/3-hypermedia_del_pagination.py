...
class Server:
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        ...
        assert isinstance(index, int) and index < (len(ind_dataset) - 1)
        i, mv, data = 0, index, []
        while (i < page_size and index < len(ind_dataset)):
            value = ind_dataset.get(mv, None)
            if value:
                data.append(value)
                i += 1
            mv += 1
